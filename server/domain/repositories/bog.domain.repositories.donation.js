...
var DonationData = require('../../data/repositories/bog.data.repositories.donation');
var PhysicalLocaitonDomain = require('../../domain/repositories/bog.domain.repositories.location.physical');
var DonationEntity = require('../../data/models/bog.data.models.donation');
var AuditEntity = require('../../data/models/bog.data.models.audit');
var DonationModelList = require('../models/bog.domain.models.donation.list');
var DonationModelSimple = require('../models/bog.domain.models.donation.simple');
var DonationModelDetail = require('../models/bog.domain.models.donation.detail');

var DonationRepository = function (current_user) {
    var self = this;
    ...
    var AllAsList = function (callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'AllAsList', function (hasAuth) {
            if (!hasAuth) {
    ...
    var AllAsSimple = function (callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'AllAsSimple', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var AllAsDetail = function (callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'AllAsDetail', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var AllAsRaw = function (callback) { // callback (errors, collection)
        self.authorization.userCanPerform(null, DonationEntity, 'AllAsRaw', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var FilteredAsList = function (filter, callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'FilteredAsList', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var FilteredAsSimple = function (filter, callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'FilteredAsSimple', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var FilteredAsDetail = function (filter, callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'FilteredAsDetail', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var FilteredAsRaw = function (filter, callback) {
        self.authorization.userCanPerform(null, DonationEntity, 'FilteredAsRaw', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var ByIdAsDetail = function (id, callback) {
        ...
        data.findById(id, function (err, entity) {
            ...
            if (!entity || entity == {}) {
                ...
                self.authorization.userCanPerform(current_user, DonationEntity, 'ByIdAsDetail', function (hasAuth) {
                    if (!hasAuth) {
                        callback(self.err.wrap(4000));
                        return;
                    }
    ...
    var ByIdAsRaw = function (id, callback) {
        ...
        data.findById(id, function (err, entity) {
            ...
            if (!entity || entity == {}) {
                ...
                self.authorization.userCanPerform(current_user, DonationEntity, 'ByIdAsRaw', function (hasAuth) {
                    if (!hasAuth) {
                        callback(self.err.wrap(4000));
                        return;
                    }
    ...
    var Add = function (input, callback) {
        self.authorization.userCanPerform(current_user, DonationEntity, 'Add', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var Update = function (id, input, callback) {  // callback(err, id)
        ...
        self.authorization.userCanPerform(current_user, DonationEntity, 'Update', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    var Delete = function (input, callback) {
        ...
        self.authorization.userCanPerform(current_user, DonationEntity, 'DeleteById', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }
    ...
    // Physical Locations
    var PhysicalLocationById = function (donationid, locid, depth, callback) {
        ...
        self.authorization.userCanPerform(current_user, DonationEntity, 'Location Get By Id', function (hasAuth) {
            if (!hasAuth) {
                callback(self.err.wrap(4000));
                return;
            }

            plocDomain.byId(locid, depth, function (err, donation_ploc) {
                if (err) {
                    callback(self.err.wrap(1001, null, err));
                } else {
                    callback(null, donation_ploc);
                }
            });
        });
    };
    var PhysicalLocationAdd = function (donationid, input, callback) {
        if (!isValidObjectID(donationid)) {
            callback(self.err.wrap(5001));
            return;
        }
        ...
        self.authorization.userCanPerform(current_user, DonationEntity, 'Update', function (hasAuth) {
            ...
            data.findById(donationid, function (err, donation_entity) {
                var parent_donation = donation_entity;
                if (err) {
                    callback(self.err.wrap(1001, null, err));
                    return;
                }
                if (!donation_entity || donation_entity == {}) {
                    callback(self.err.wrap(5005));
                } else {
                    plocDomain.add(input, function (err, saved_ploc) {
                        if (err) {
                            ...
                            parent_donation.locations_physical.push(saved_ploc);
                            data.update(donationid, parent_donation, function (err, saved_entity) {
                                if (err) {
                                    callback(self.err.wrap(1001, null, err));
                                } else {
    ...
    var PhysicalLocationUpdate = function (donationid, locid, input, callback) {
        if (!isValidObjectID(donationid)) {
            callback(self.err.wrap(5001));
            return;
        }
    ...
    var PhysicalLocationRemove = function (donationid, locid, callback) {
        if (!isValidObjectID(donationid)) {
            callback(self.err.wrap(5001));
            return;
        }
                ...
                if (!donation_entity || donation_entity == {}) {
                    ...
                    plocDomain.delete(locid, function (err, saved_ploc) {
                        if (err) {
                            ...
                            parent_donation.locations_physical.pop(saved_ploc);
                            data.update(donationid, parent_donation, function (err, saved_entity) {
                                if (err) {
                                    callback(self.err.wrap(1001, null, err));
                                } else {
    ...
    var validate = function (entity, callback) {
        console.log('TODO: Add domain logic for: donation');
        callback(null, entity);
    };
    ...
    var pushRawCollectionToList = function (collection, callback) {
        ...
        for (var i = 0; i < collection.length; i++) {
            ...
            var listModel = new DonationModelList().fromRaw(rawItem);
    ...
    var pushRawCollectionToSimple = function (collection, callback) {
        ...
        for (var i = 0; i < collection.length; i++) {
            var rawItem = collection[i];
            var simpleModel = new DonationModelSimple().fromRaw(rawItem);
    ...
    var pushRawCollectionToDetail = function (collection, callback) {
        ...
        for (var i = 0; i < collection.length; i++) {
            var rawItem = collection[i];
            var detailModel = new DonationModelDetail().fromRaw(rawItem);
