        ...
        serializeAddress(location, function (serialized_location) {
            geo.code(serialized_location, function (geocode) {
                if (geocode.results.length == 0) {
                    callback(self.err.wrapForResponse(400, 'Address Verification failed.  No match found.'), false);
                    return;
                }
                if (geocode.results.length > 1) {
                    callback(self.err.wrapForResponse(400, 'Address Verification failed.  Multiple results found.'), false);
                    return;
                }
                if (geocode.status != 'OK') {
                    callback(self.err.wrapForResponse(400, 'Address Verification failed.  Invalid return staus: ' + geocode.status), false);
                    console.log("status:  " + geocode.status);
                    return;
                }
                if (geocode.results[0].partial_match == true) {
                    callback(self.err.wrapForResponse(400, 'Address Verification failed.  Only partial matches found.'), false);
                    return;
                }
                if (geocode.results.length == 1) {
